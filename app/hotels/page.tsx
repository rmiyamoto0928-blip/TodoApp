...
import { Hotel, SortOption } from '@/lib/types'
import HotelCard from '@/components/hotels/HotelCard'
import SearchBar from '@/components/ui/SearchBar'
import FilterTags from '@/components/ui/FilterTags'
import SortDropdown from '@/components/ui/SortDropdown'
import { sortHotels, searchFilter, HOTEL_CATEGORIES } from '@/lib/utils'
...
export default function HotelsPage() {
  ...
  if (cats.length > 0) filtered = filtered.filter((h) => cats.includes(h.category))
  if (favOnly) filtered = filtered.filter((h) => h.isFavorite)
  const sorted = sortHotels(filtered, rankMode ? 'rating-desc' : sort)

  return (
    <div className="flex flex-col min-h-screen">
      <div className="sticky top-0 z-40 bg-[#f8fafc] px-4 pt-4 pb-2 space-y-2">
        <div className="flex items-center justify-between">
          <h1 className="text-xl font-bold text-gray-900">ホテル 🏨</h1>
        ...
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-1">
          ...
          <button onClick={() => setShowFilter(!showFilter)}
            className={`shrink-0 text-xs px-3 py-1.5 rounded-full font-medium transition-all ${cats.length > 0 ? 'bg-sky-400 text-white' : 'bg-white text-gray-500 border border-gray-200'}`}>
            🏷 カテゴリ{cats.length > 0 ? ` (${cats.length})` : ''}
          </button>
          {!rankMode && <SortDropdown value={sort} onChange={setSort} />}
        </div>
        {showFilter && <FilterTags genres={HOTEL_CATEGORIES} selected={cats} onChange={setCats} />}
      </div>
      ...
      <div className="px-4 pb-4">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-gray-400 gap-3">
            ...
            <p className="text-sm">{items.length === 0 ? 'まだ記録がありません' : '条件に合うホテルがありません'}</p>
            {items.length === 0 && (
              <Link href="/hotels/new" className="text-sky-400 text-sm font-medium">最初のホテルを追加 →</Link>
            )}
          </div>
        ) : (
          <div className="grid grid-cols-2 gap-3">
            {sorted.map((item, i) => (
              <HotelCard key={item.id} item={item} rank={rankMode ? i + 1 : undefined} onFavoriteToggle={toggleFavorite} />
            ))}
          </div>
        )}
      </div>
