...
export default function HotelCard({ item, rank, onFavoriteToggle }: HotelCardProps) {
      ...
      <Link href={`/hotels/${item.id}`} className="block">
        ...
        <div className="p-3">
          ...
          <div className="flex items-center gap-1 mt-1.5">
            <CarRating rating={Math.round(overall)} size="sm" />
            <span className="text-xs text-gray-400">({overall})</span>
          </div>
          <div className="flex flex-wrap gap-2 mt-1.5 text-xs text-gray-500">
            <span>🍽 {'🚗'.repeat(item.ratingFood)}</span>
            <span>🛁 {'🚗'.repeat(item.ratingBath)}</span>
            <span>🛏 {'🚗'.repeat(item.ratingRoom)}</span>
          </div>
